...
const RoomGallery = ({ isOpen, onClose }: RoomGalleryProps) => {
  ...
  // Load room gallery items from translations
  const roomGalleryData = t("facilities.room_gallery", { returnObjects: true }) as Array<{
    alt: string;
    title: string;
  }>;

  const roomImages = [
    { src: overwaterVillaBedroom, alt: roomGalleryData?.[0]?.alt ?? "Luxury Overwater Villa Bedroom", title: roomGalleryData?.[0]?.title ?? "Overwater Villa Suite" },
    { src: luxuryBathroomOceanView, alt: roomGalleryData?.[1]?.alt ?? "Luxury Bathroom with Ocean View", title: roomGalleryData?.[1]?.title ?? "Ocean View Bathroom" },
    { src: suiteLivingArea, alt: roomGalleryData?.[2]?.alt ?? "Suite Living Area", title: roomGalleryData?.[2]?.title ?? "Spacious Living Area" },
    { src: privateBalcony, alt: roomGalleryData?.[3]?.alt ?? "Private Balcony", title: roomGalleryData?.[3]?.title ?? "Private Ocean Balcony" },
    { src: twinBedroom, alt: roomGalleryData?.[4]?.alt ?? "Twin Bedroom", title: roomGalleryData?.[4]?.title ?? "Twin Bed Configuration" },
    { src: workspaceArea, alt: roomGalleryData?.[5]?.alt ?? "Workspace Area", title: roomGalleryData?.[5]?.title ?? "In-Room Workspace" },
    { src: dressingArea, alt: roomGalleryData?.[6]?.alt ?? "Dressing Area", title: roomGalleryData?.[6]?.title ?? "Walk-in Closet" },
    { src: roomEntrance, alt: roomGalleryData?.[7]?.alt ?? "Room Entrance", title: roomGalleryData?.[7]?.title ?? "Elegant Room Entrance" },
  ];
      ...
      <div className="relative w-full h-full max-w-7xl mx-auto p-4">
        ...
        {isLoading && (
          <div className="absolute inset-0 flex items-center justify-center z-30">
            <div className="text-center">
              ...
              <p className="text-white text-lg font-semibold">{Math.round((loadedImages / roomImages.length) * 100)}%</p>
            </div>
          </div>
        )}
        ...
        <div className={`md:hidden w-full h-full flex flex-col justify-center items-center px-4 ${isLoading ? "opacity-0" : "opacity-100"} transition-opacity duration-300`}>
          <div className="text-center mb-2">
            <h3 className="text-white text-lg font-semibold">{roomImages[currentImageIndex].title}</h3>
          ...
          <div className="text-center mt-2">
            <Button onClick={nextImage} variant="accent" size="lg" className="text-lg px-8 py-4">
              {t("facilities.gallery_next")}
            </Button>
          </div>
        </div>
        ...
        <div className={`hidden md:block lg:hidden ${isLoading ? "opacity-0" : "opacity-100"} transition-opacity duration-300`}>
          <h2 className="text-white text-3xl font-bold text-center mb-8">{t("facilities.gallery_heading")}</h2>
          ...
          <div className="grid grid-cols-3 grid-rows-3 gap-4 max-h-[calc(100vh-200px)] overflow-y-auto">
            {roomImages.slice(0, 9).map((image, index) => (
              <div key={index} className="aspect-square relative cursor-pointer overflow-hidden rounded-lg" onClick={() => setCurrentImageIndex(index)}>
                <img
                  src={image.src}
                  alt={image.alt}
